...
void WaypointReplanner::replanLaneWaypointVel(autoware_msgs::Lane& lane)
{
  ...
  if (config_.replan_curve_mode)
  {
    ...
    {// set velocity_max for all_point为所有轨迹点设置最大速度
      setVelocityByRange(0, last, 0, config_.velocity_max, lane);
    }
    // set velocity by curve通过曲线设置速度
  ...
  // set velocity on start & end of lane设置lane的起始和末尾点的速度
...
void WaypointReplanner::resampleLaneWaypoint(const double resample_interval, autoware_msgs::Lane& lane, LaneDirection dir)
{
  ...
  {
    CbufGPoint curve_point = getCrvPointsOnResample(lane, original_lane, i);//采样三个轨迹点
    //由三个轨迹点求轨迹的圆心和半径
...
//将每个轨迹点（除去第一个和最后一个轨迹点）的半径存入传入函数的变量curve_radius中
...
// get curve 3-Parameter [center_x, center_y, radius] with 3 point input. If error occured, return empty vector.计算圆心和半径
