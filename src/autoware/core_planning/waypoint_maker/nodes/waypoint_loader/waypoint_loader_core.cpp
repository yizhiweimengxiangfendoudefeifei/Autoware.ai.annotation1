...
//将paths中各个本地路径对应文件中包含的信息分别填入lane中，再将lane依次填入lane_array
...
//主要作用是首先检查文件file_path中数据是否合规，接着读取文件中的数据存入lane中
...
//讲line中的数据由string转化为double类型后存入wp中
...
//以“，”作为分隔符将字符串Line分为若干段，并将其中的空格全部去除，依次存储至字符串向量columns中
