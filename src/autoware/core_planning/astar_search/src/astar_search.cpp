...
// state update table for hybrid astar创造更新状态是本章的重点
...
bool AstarSearch::setStartNode(const geometry_msgs::Pose& start_pose)
{
  ...
  start_node.hc = nodes_[index_y][index_x][0].hc;//feifei add
  start_node.gc = 0;
...
bool AstarSearch::setGoalNode(const geometry_msgs::Pose& goal_pose)
{
  ...
  // Check if goal is valid，检查下标是否溢出
...
//根据传入函数的pose确定对应地图节点在三维代价地图nodes_中的坐标
...
bool AstarSearch::search()
{
  ...
  while (!openlist_.empty())
  {
    ...
    // Pop minimum cost node from openlist从openlist移出最低代价节点
